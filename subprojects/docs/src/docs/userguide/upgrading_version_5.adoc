...
This affects Kotlin DSL build scripts that make use of e.g. extra properties inside the `dependencies {}` block. They will now resolve to extra properties on `dependencies` itself. In order to address `Project` extra properties inside the `dependencies {}` block you need to explicitely use `project.extra` instead of just `extra`.
